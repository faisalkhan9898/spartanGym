  ...
  return (
    <section className={`py-20 sm:py-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-slate-950 text-white border-t border-slate-800 ${className}`}>
      ...
      <div className="relative z-10 max-w-4xl mx-auto text-center space-y-4 sm:space-y-6">
        ...
        <div className="inline-flex items-center gap-1.5 sm:gap-2 px-3 py-1 sm:px-4 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-black uppercase tracking-widest bg-yellow-400/20 text-yellow-300 border border-yellow-400/40 backdrop-blur-md shadow-sm">
          <Flame className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-yellow-400 animate-pulse" />
          <span>Limited Time Introductory Offer</span>
        </div>

        <h2 className="text-2xl xs:text-3xl sm:text-5xl lg:text-6xl font-black text-white uppercase font-heading tracking-tight leading-tight">
          {title}
        </h2>
        ...
        <div className="pt-2 sm:pt-4 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
          <Button to="/join" variant="primary" size="lg" className="w-full sm:w-auto text-xs sm:text-base" icon={ChevronRight}>
            Claim Your Membership
          </Button>
          {showTrial && (
            <Button to="/free-trial" variant="white" size="lg" className="w-full sm:w-auto text-xs sm:text-base">
